...
export default function ModuleTile({
  ...
  return (
    <Link href={`/modules/${name}`} className={classes.tileWrapper}>
      <li
        className={classNames(classes.moduleTile, "shadow-xl z-40 rounded-lg border-2 border-solid shadow-md bg-teal-50")}
      >
        ...
        {description && <div className={classNames(classes.description, "bg-pink-100 rounded-lg border-2 border-solid")}>
          description: {description}
        </div>}
        {attributes && (
          <div className={classes.attributes}>
            {attributes.map((attribute) => (
              <div key={attribute} className={classNames(classes.attribute, "bg-yellow-300 rounded-lg border-2 border-solid")}>
                {attribute}
              </div>
            ))}
          </div>
